...
주관적 "그만두고 싶다" 신호 대신 **객관 패턴**으로 판단.

### 규칙
```
최근 3문제의 소요시간이 "감소 → 증가"로 꺾이면 종료 신호
= t[n-2] > t[n-1] AND t[n-1] < t[n]
= V자 바닥 찍고 상승하는 순간
```

### 적용 조건
1. **같은 레벨 내에서만 비교** (Lv.0과 Lv.1 섞으면 해석 무의미)
2. **최소 3문제 이상** 풀었을 때부터 적용 (세션 초반엔 측정 불가)
3. **1회 신호는 경고, 연속 신호는 확정**
   - V자 반전 감지 → 한 문제 더 해보고
   - 다음도 유지/증가면 **확정 종료**
   - 다음이 다시 짧아지면 **noise로 간주**하고 계속

### 발견 근거 (Day 2 데이터)
Lv.1 세션: 19 → 10 → 8 → 7 → **14**
- 4번째까지 학습 가속 (19→7)
- 5번째에서 역전 (7→14)
- 같은 시점에 주관적 "그만두고 싶다" 신호 발생
- → 객관 패턴과 주관 신호 일치 확인
...
### 한계
- "문제가 본질적으로 어려웠다"와 "내 집중력 저하"의 구분은 1회 신호만으론 불가
- → **연속 확인**으로 구분. 연속성이 핵심.
